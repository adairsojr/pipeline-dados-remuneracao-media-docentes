{{ config(location='../data/gold/fato_maiores_remuneracoes.parquet') }}
...
ranking_maiores AS (
    SELECT 
        ano,
        uf,
        ROW_NUMBER() OVER (PARTITION BY ano ORDER BY AVG(valor) DESC) AS posicao
    FROM base
    GROUP BY ano, uf
)
